...
export async function getRewards(merchantId?: string): Promise<Reward[]> {
  const m = merchantId?.trim();
  return get<Reward[]>(`/rewards${m ? `?merchant_id=${encodeURIComponent(m)}` : ''}`);
}
...
export async function getCoupons(merchantId?: string): Promise<Coupon[]> {
  const m = merchantId?.trim();
  return get<Coupon[]>(`/coupons${m ? `?merchant_id=${encodeURIComponent(m)}` : ''}`);
}
...
export async function getVouchers(merchantId?: string): Promise<GiftVoucher[]> {
  const m = merchantId?.trim();
  return get<GiftVoucher[]>(`/vouchers${m ? `?merchant_id=${encodeURIComponent(m)}` : ''}`);
}
...
export async function getPointsRules(merchantId?: string): Promise<PointsRule[]> {
  const m = merchantId?.trim();
  return get<PointsRule[]>(`/points-rules${m ? `?merchant_id=${encodeURIComponent(m)}` : ''}`);
}
