...
const STATUS_CFG = {
  merchant_allocated: { cls: 'bg-[#F5EDD0]/30 text-[#B8941F]', label: 'Available', icon: 'check_circle' },
  ...
  return (
    <div className="px-4 md:px-10 py-8 max-w-5xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Total Card', value: stats.total, icon: 'credit_card', cls: 'bg-[#F5EDD0]/20 text-[#B8941F]' },
          { label: 'Balance', value: stats.available, icon: 'check_circle', cls: 'bg-[#F3F4F6] text-[#6B7280]' },
          { label: 'Active Member', value: stats.linked, icon: 'person_check', cls: 'bg-[#F3F4F6] text-[#6B7280]' },
        ].map(s => (
      ...
      {!loading && cards.length === 0 && (
        <div className="card p-6 flex flex-col items-center text-center py-16">
          <div className="w-20 h-20 bg-[#F3F4F6] rounded-2xl flex items-center justify-center mb-4">
            <span className="material-symbols-outlined text-[#6B7280] text-[40px]">credit_card_off</span>
          </div>
          <h3 className="text-xl font-bold mb-2">No cards allocated yet</h3>
          <p className="text-sm text-[#6B7280] max-w-sm">Contact your Metro Cardz admin to allocate a batch of physical cards to your store. Once allocated, they'll appear here for you to link to members.</p>
        </div>
      )}
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-sm transition-all
                    ${filterStatus === f.key ? 'bg-[#B8941F] text-white shadow-sm' : 'text-[#6B7280] hover:bg-[#F3F4F6]-high'}`}
                >
                  {f.label}
                </button>
              ))}
            ...
            {loading ? (
              Array.from({ length: 6 }).map((_, i) => (
                <div key={i} className="card p-4 animate-pulse">
                  <div className="h-5 bg-[#F3F4F6] rounded mb-3 w-2/3" />
              ...
              return (
                <div
                  ...
                  className={`card p-4 flex items-center gap-4 group transition-all
                    ${isLinked ? 'border-l-4 border-[#6B7280]' : 'border-l-4 border-[#B8941F]-container/50'}`}
                >
                  {/* Card visual */}
                  <div className={`w-14 h-10 rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm
                    ${isLinked ? 'bg-[#6B7280]' : 'bg-[#B8941F]'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      <span className="font-mono font-bold text-sm tracking-wider">{card.card_number}</span>
                      {cfg && (
                    ...
                    {isLinked ? (
                      <div>
                        <p className="text-sm font-bold text-[#111111]">{card.linked_member_name}</p>
                        <p className="text-xs text-[#6B7280]">{card.linked_member_phone}</p>
                        {card.linked_at && (
                          <p className="text-xs text-[#6B7280]">Linked {format(new Date(card.linked_at), 'dd MMM yyyy')}</p>
                        )}
                      </div>
                    ) : (
                      <p className="text-xs text-[#6B7280]">Not yet linked to any member</p>
                    )}
                  </div>
                    ...
                    {isLinked ? (
                      <>
                        <button
                          ...
                          className="text-xs text-[#B8941F] hover:bg-[#F5EDD0]/20 px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"
                        >
                          View Member
                        </button>
                        {isOwner && (
                          <button
                            ...
                            className="text-xs text-[#6B7280] hover:bg-red-50 hover:text-red-700 px-3 py-1.5 rounded-lg transition-colors"
                          >
                            Unlink
                          </button>
                        )}
        ...
        <div className="fixed inset-0 z-[900] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-lg mx-4 animate-scale-in space-y-5">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold">Assign Card to Member</h3>
              <button onClick={() => !linking && setLinkTarget(null)} className="text-[#6B7280] hover:text-[#111111]">
                <span className="material-symbols-outlined">close</span>
              </button>
            </div>

            {/* Card preview */}
            <div className="bg-[#B8941F] rounded-xl p-4 flex items-center gap-3 text-white">
              <span className="material-symbols-outlined text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>credit_card</span>
              <div>
                <p className="text-xs opacity-70">Assigning card</p>
                <p className="font-mono font-bold text-xl tracking-widest">{linkTarget.card_number}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <p className="text-xs text-[#6B7280] mt-1">Only members without an existing card are shown.</p>
            </div>
                ...
                {memberResults.map(m => (
                  ...
                  >
                    ...
                    {linking ? (
                      ...
                      <span className="material-symbols-outlined text-[#B8941F]">arrow_forward</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            {memberSearch.length >= 2 && !memberSearching && memberResults.length === 0 && (
              <div className="text-center py-6 text-[#6B7280]">
                ...
                <p className="text-sm">No members found — or all members already have a card</p>
              </div>
            )}
          </div>
        </div>
        ...
        description={
          <div className="space-y-3">
            <div className="bg-[#F3F4F6] rounded-xl p-4">
              ...
              <p className="text-center text-[#6B7280] text-xs mt-1">Currently linked to: <strong>{unlinkTarget?.linked_member_name}</strong></p>
            </div>
